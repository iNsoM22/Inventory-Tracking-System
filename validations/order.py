...
class CartItemBase(BaseModel):
    ...
    quantity: int = Field(
        gt=0, description="Quantity of the Product in the Cart")
...
class CartItemUpdateRequest(CartItemBase):
    quantity: Optional[int] = Field(
        None, ge=0, description="Updated Quantity of the Product in the Cart")
    discount: Optional[float] = Field(
        None, ge=0, description="Updated Discount Rate of the Product in the Cart")
...
class CartItemResponse(CartItemBase):
    ...
    product_id: UUID4 = Field(
        exclude=True, description="Unique identifier for the Product")
...
class OrderBase(BaseModel):
    ...
    discount_amount: float = Field(
        ge=0, description="Discounted Amount of the Order")
    ...
    date_placed: Optional[datetime] = Field(
        default=None, description="Timestamp When the Order is Placed")
    date_received: Optional[datetime] = Field(
        default=None, description="Timestamp When the Order is Delivered to the Customer")
    order_mode: Literal["Online", "Offline"] = Field(
        default="Offline", description="Order mode, either Online or Offline")
    order_delivery_address: Optional[str] = Field(
        None, description="Delivery Address of the Customer")
    customer_id: UUID4 = Field(...,
                               description="Unique identifier for the Customer")
    items: List[CartItemBase] = Field(
        strict=True, description="List of Cart Items in the Order")
...
class OrderRequest(OrderBase):
    store_id: UUID4 = Field(...,
                            description="Store ID from Where the Order is Placed")
...
class OrderResponse(OrderBase):
    ...
    customer_id: UUID4 = Field(
        exclude=True, description="Unique identifier for the Customer")


class OrderResponseWithOutStore(OrderResponse):
    store_id: UUID4 = Field(
        exclude=True, description="Store Identifier where the Order is Placed")


class OrderResponseWithCustomer(OrderResponse):
    customer: Optional[CustomerResponse] = Field(
        None, description="Customer Details")
